    ...
    public partial class frmAdmAnotacion : Form
    {
        ...
        private void gridView1_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            ...
            { 
                ...
                Correlativo1 = (view.GetRowCellDisplayText(row[0], view.Columns[9])).ToString();
                Sucursal = (view.GetRowCellDisplayText(row[0], view.Columns[12])).ToString();
                Manifiesto = (view.GetRowCellDisplayText(row[0], view.Columns[15])).ToString();
        ...
        private void iconButton1_Click(object sender, EventArgs e)
        {
            ...
            if (dtsAnotacionDet.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    string status = dtsAnotacionDet.Tables[0].Rows[i]["Status"].ToString().Trim();
                    if (status == "OPEN")
                    {
                        ...
                        oEtiqueta.SetParameterValue("calidad", dtsAnotacionDet.Tables[0].Rows[i]["Calidad"].ToString().Trim());
                        oEtiqueta.SetParameterValue("colada", dtsAnotacionDet.Tables[0].Rows[i]["Colada"].ToString().Trim());
                        oEtiqueta.SetParameterValue("pieza", dtsAnotacionDet.Tables[0].Rows[i]["Piezas"].ToString().Trim());
                        oEtiqueta.SetParameterValue("etiqueta", dtsAnotacionDet.Tables[0].Rows[i]["CodigoBarra"].ToString().Trim());
                        oEtiqueta.SetParameterValue("medida", dtsAnotacionDet.Tables[0].Rows[i]["CodPackList"].ToString().Trim());
                        oEtiqueta.SetParameterValue("correlativo", dtsAnotacionDet.Tables[0].Rows[i]["Correlativo"].ToString().Trim());
                        oanotaciondet.UpdateAnotacionDet(dtsAnotacionDet.Tables[0].Rows[i]["CodigoBarra"].ToString().Trim(), "IMPR");
                        oEtiqueta.PrintToPrinter(2, true, 1, 1);
