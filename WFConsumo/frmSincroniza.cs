...
namespace WFConsumo
{
    public partial class frmSincroniza : DevExpress.XtraEditors.XtraForm
    {
        ...
        bool EnvioOK = false;
         ...
         public frmSincroniza(int Sucursal, string centro, string sOrden)
        {
            iSucursal = Sucursal;
            cSuc = new CSucursal();
            ...
            edit.ButtonClick += (sender, e) =>
            {
                if (e.Button.Kind == ButtonPredefines.Ellipsis)
                {
                    ...
                    //DataView r = (DataView)gridView1.GetFocusedRow();
                    if (row["STATUS"].ToString() == "OPEN")
                    {
                        if (XtraMessageBox.Show("Desea sincronizar este paquete?", "Confirmation Dialog", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
                            EnviarPaquete(row["PAQUETEID"].ToString());
                        }
                    }
                    else
                        XtraMessageBox.Show("Paquete ya ha sido integrado");
                }
            };
            edit.Buttons[0].Kind = ButtonPredefines.Ellipsis;
            edit.Buttons[0].Caption = "Custom Button";
            dgvProductos.RepositoryItems.Add(edit);
            GridColumn column = gridView1.Columns["PAQUETEID"];
            column.ColumnEdit = edit;
            column.ShowButtonMode = ShowButtonModeEnum.ShowAlways;
        }
        void EnviarPaquete(string strPaquete)
        {
            try
            {
        ...
        private void CargarConsumos()
        {
            ...
            gridView2.Columns["FCS_ID"].Visible = false;
            gridView2.Columns["FCS_BATCH_LABEL_1"].VisibleIndex = 0;

            if (gridView2.Columns["PAQUETEID"].SummaryItem.Collection.Count == 1)
            {
                GridColumnSummaryItem ConsCount = new GridColumnSummaryItem();
                ConsCount.SummaryType = SummaryItemType.Count;
                ConsCount.DisplayFormat = "{0}";
                gridView2.Columns["PAQUETEID"].SummaryItem.Collection.Add(ConsCount);
            }
            if (gridView2.Columns["CANTIDAD_CONSUMIDA"].SummaryItem.Collection.Count == 1)
            {
                GridColumnSummaryItem ConsTotal = new GridColumnSummaryItem();
                ConsTotal.SummaryType = SummaryItemType.Sum;
                ConsTotal.DisplayFormat = "{0}";
                gridView2.Columns["CANTIDAD_CONSUMIDA"].SummaryItem.Collection.Add(ConsTotal);
            }
            //var sumCSMD = dtC.AsEnumerable().Sum(dr => dr.Field<decimal>("Cantidad_Consumida"));
        }
        private void CargarProductos()
        {
            ...
            gridView1.Columns["CENTRO_TRABAJO"].Visible = false;
            gridView1.Columns["NRO_ORDEN"].Visible = false;
            gridView1.Columns["METROS"].Visible = false;
            gridView1.Columns["FBC_BATCH_LABEL_1"].Visible = false;
            gridView1.Columns["FBC_BATCH_LABEL_2"].Visible = false;
            gridView1.Columns["POSICION"].VisibleIndex = 0;
            gridView1.Columns["CostoUnitario"].VisibleIndex = 6;
            gridView1.Columns["CostoTotal"].VisibleIndex = 7;
            if (gridView1.Columns["PAQUETEID"].SummaryItem.Collection.Count == 1)
            {
                GridColumnSummaryItem ProdCount = new GridColumnSummaryItem();
                ProdCount.SummaryType = SummaryItemType.Count;
                ProdCount.DisplayFormat = "{0}";
                gridView1.Columns["PAQUETEID"].SummaryItem.Collection.Add(ProdCount);
            }
            if (gridView1.Columns["PESO"].SummaryItem.Collection.Count == 1)
            {
                GridColumnSummaryItem ProdTotal = new GridColumnSummaryItem();
                ProdTotal.SummaryType = SummaryItemType.Sum;
                ProdTotal.DisplayFormat = "{0}";
                gridView1.Columns["PESO"].SummaryItem.Collection.Add(ProdTotal);
            }
            if (gridView1.Columns["CostoTotal"].SummaryItem.Collection.Count == 1)
            {
                GridColumnSummaryItem ProdTotal = new GridColumnSummaryItem();
                ProdTotal.SummaryType = SummaryItemType.Sum;
                ProdTotal.DisplayFormat = "{0}";
                gridView1.Columns["CostoTotal"].SummaryItem.Collection.Add(ProdTotal);
            }
        ...
        private void frmSincroniza_Load(object sender, EventArgs e)
        {
        ...
        void Simulacion()
        {
            ...
            if (gridView3.Columns["Peso"].SummaryItem.Collection.Count == 1)
            {
                ...
                CsmdTotal.DisplayFormat = "{0}";
                gridView3.Columns["Peso"].SummaryItem.Collection.Add(CsmdTotal);
            }
        ...
        private void btnBajar_Click(object sender, EventArgs e)
        {
            ...
            if (rowHandle < this.gridView1.RowCount - 1)
            {
                paqOrigen = this.gridView1.GetRowCellValue(rowHandle, "PAQUETEID").ToString();
                paqDestino = this.gridView1.GetRowCellValue(rowHandle + 1, "PAQUETEID").ToString();
                ActualizaPosicion(strOrden, paqOrigen, paqDestino, rowHandle);
                this.gridView1.FocusedRowHandle = rowHandle+1;
            }
        }
        ...
        private void dgvConsumido_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.G && e.Control)
            {
        ...
        private void dgvConsumos_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.G && e.Control)
            {
        ...
        {
            string val = gridView1.
                GetRowCellValue(gridView1.FocusedRowHandle, "STATUS").ToString();
            e.Cancel = gridView1.FocusedColumn.FieldName != "PAQUETEID" ||
                val=="SYNC";
        }
        ...
        private void frmSincroniza_Resize(object sender, EventArgs e)
        {
            ReDimensionar();
        }
